...
function runTests() {
  ...
  console.assert(students.length === 1248, `Expected 1248 students, got ${students.length}`);
  console.log(`✓ Total Students: ${students.length} (Expected: 1248)`);

  console.assert(parents.length === 850, `Expected 850 parents, got ${parents.length}`);
  ...
  if (riya) {
    console.assert(riya.admissionNo === '1088', `Riya Admission expected 1088, got ${riya.admissionNo}`);
    console.assert(riya.className === 'Class 8-A', `Riya Class expected Class 8-A, got ${riya.className}`);
  ...
  if (raj) {
    ...
    console.assert(Math.abs(raj.paymentReliabilityBreakdown.onTimeRate - 82.0) < 1.0, `Raj On-Time Rate expected ~82%, got ${raj.paymentReliabilityBreakdown.onTimeRate}`);
  ...
  if (priya) {
    ...
    console.assert(priyaRank === 1, `Priya Rank expected #1, got #${priyaRank}`);
    console.assert(Math.abs(priya.performanceBreakdown.score - 91.4) < 1.5, `Priya Index expected ~91.4, got ${priya.performanceBreakdown.score}`);
  ...
  console.assert(Math.abs(collected - 14200000) < 1, `Collected expected ₹1.42 Cr, got ${collected}`);
  console.log(`✓ Collected Fees: ₹${(collected / 10000000).toFixed(2)} Cr`);

  console.assert(Math.abs(outstanding - 4230000) < 1, `Outstanding expected ₹42.3 L, got ${outstanding}`);
  console.log(`✓ Total Outstanding: ₹${(outstanding / 100000).toFixed(1)} L`);

  console.assert(Math.abs(aging.currentNotDue - 1250000) < 1, `Current/Not Due expected ₹12.5 L, got ${aging.currentNotDue}`);
  console.log(`✓ Current / Not Due: ₹${(aging.currentNotDue / 100000).toFixed(1)} L`);

  console.assert(Math.abs(overdue - 2980000) < 1, `Overdue expected ₹29.8 L, got ${overdue}`);
  console.log(`✓ Total Overdue: ₹${(overdue / 100000).toFixed(1)} L`);

  console.assert(Math.abs(aging.days0_30 - 800000) < 1, `0-30 days expected ₹8.0 L, got ${aging.days0_30}`);
  console.log(`✓ 0-30 Days: ₹${(aging.days0_30 / 100000).toFixed(1)} L`);

  console.assert(Math.abs(aging.days31_60 - 900000) < 1, `31-60 days expected ₹9.0 L, got ${aging.days31_60}`);
  console.log(`✓ 31-60 Days: ₹${(aging.days31_60 / 100000).toFixed(1)} L`);

  console.assert(Math.abs(aging.days61_90 - 680000) < 1, `61-90 days expected ₹6.8 L, got ${aging.days61_90}`);
  console.log(`✓ 61-90 Days: ₹${(aging.days61_90 / 100000).toFixed(1)} L`);

  console.assert(Math.abs(aging.days90Plus - 600000) < 1, `90+ days expected ₹6.0 L, got ${aging.days90Plus}`);
  ...
  console.assert(Math.abs(agingOverdueSum - overdue) < 1, `Aging overdue sum (${agingOverdueSum}) does not equal total overdue (${overdue})`);
  console.log(`✓ Aging Overdue Sum: ₹${(agingOverdueSum / 100000).toFixed(1)} L`);

  const currentPlusOverdue = aging.currentNotDue + overdue;
  console.assert(Math.abs(currentPlusOverdue - outstanding) < 1, `Current + Overdue (${currentPlusOverdue}) does not equal total outstanding (${outstanding})`);
  console.log(`✓ Current + Overdue: ₹${(currentPlusOverdue / 100000).toFixed(1)} L`);

  console.assert(Math.abs(expected - (collected + outstanding)) < 1, `Expected fees (${expected}) does not equal collected + outstanding (${collected + outstanding})`);
